...
class ThreadPoolUtils {
    ...
    private lateinit var scheduleExec: ScheduledExecutorService
     ...
     */
    constructor(@Type type: Int, corePoolSize: Int) {
        // 构造有定时功能的线程池
    ...
    @Throws(InterruptedException::class)
    fun <T> invokeAll(tasks: Collection<Callable<T>>,
                      timeout: Long,
    ...
    fun <T> invokeAny(tasks: Collection<Callable<T>>,
                      timeout: Long,
                      unit: TimeUnit): T {
    ...
    fun <V> schedule(callable: Callable<V>,
                     delay: Long,
     ...
     */
    fun scheduleWithFixedRate(command: Runnable,
                              initialDelay: Long,
                              period: Long,
                              unit: TimeUnit): ScheduledFuture<*> {
