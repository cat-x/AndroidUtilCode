...
object ImageUtils {
    ...
    fun getBitmap(data: ByteArray, offset: Int): Bitmap? {
        return if (data.isEmpty()) null else BitmapFactory.decodeByteArray(data, offset, data.size)
    }
    ...
    fun stackBlur(src: Bitmap, radius: Int, recycle: Boolean = false): Bitmap {
        ...
        while (y < h) {
            ...
            while (x < w) {
                ...
                stackstart = stackpointer - radius1 + div
                ...
                if (y == 0) {
                    vmin[x] = Math.min(x + radius1 + 1, wm)
                }
                p = pix[yw + vmin[x]]
